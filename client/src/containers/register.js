...
import { authUserRegister } from "./auth.actions";
...
class Register extends Component {
	...
	onBlurHandler = ({target}) =>{
		...
		this.props.authUserRegister({ s_id:parseInt(localStorage.getItem("s_id")),select_courses:parseInt(this.state.select_courses),select_batch:parseInt(this.state.select_batch),objective_learning:this.state.objective_learning,flag : 0});
	  }
    ...
    componentWillReceiveProps(nextProps) {
		...
		else if(this.props.setAllCoursesResponseIdentifer != nextProps.setAllCoursesResponseIdentifer){
			 window.$("#myModal1").modal("show");
			 if(!!localStorage.getItem('s_id')){
				this.setState({isAuth:true,courses:nextProps.data});
			 }
			 else{
