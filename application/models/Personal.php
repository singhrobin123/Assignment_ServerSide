...
class Personal extends CI_Model {
    ...
    public function UserModel($s_id) {
        ...
        $data = ['flag' => true, 'User' => $data, 'courses' => $courses];
        return $data;
    }
    public function registerModel($email, $password, $first_name, $last_name, $batch_id, $objective_of_learning, $course_id) {
        ...
        $dc = $query->result();
        if ($dc) {
            $data = ['flag' => false, 'users' => [], 'Courses' => false, 's_id' => null, 'fname' => null, 'lname' => null, 'message' => 'Email already exist', 'password' => false, 'email' => false];
            return $data;
        } else {
            if ($password == "" || $batch_id == "") {
                $data = ['flag' => false, 'users' => null, 'Courses ' => false, 's_id' => null, 'fname' => null, 'lname' => null, 'message' => 'Not successfully registered'];
                return $data;
            } else {
                ...
                if ($query) {
                    ...
                    $query = $this->db->query($sql);
                    $Courses = $query->result();
                    $data = ['flag' => true, 'users' => $users, 'Courses ' => $Courses, 's_id' => $s_id, 'fname' => $fname, 'lname' => $lname, 'message' => 'successfully registered'];
    ...
    public function courseModel($s_id, $batch_id, $objective_of_learning, $course_id) {
        ...
        foreach ($query->result_array() as $row) {
            ...
            if ($x == $course_id) {
                $f = 1;
                $data = ['flag' => false, 'Course' => false, 's_id' => $s_id, 'fname' => $fname, 'lname' => $lname, 'message' => 'Course already registered', ];
                return $data;
            }
        }
        if ($batch_id == "") {
            ...
            if ($query) {
                $data = ['flag' => true, 'Course' => true, 's_id' => $s_id, 'fname' => $fname, 'lname' => $lname, 'message' => 'successfully registered', ];
                return $data;
            } else {
                $data = ['flag' => false, 'Course' => false, 's_id' => $s_id, 'fname' => $fname, 'lname' => $lname, 'message' => 'something went wrong', ];
                return $data;
            }
        }
    }
    public function loginModel($email, $password) {
        ...
        if ($query->result()) {
            ...
            if ($pass[0]->password == $password) {
                ...
                $query = $this->db->query($sql);
                $Courses = $query->result();
                $data = ['flag' => true, 'users' => $users, 'Courses' => $Courses, 's_id' => $s_id, 'fname' => $fname, 'lname' => $lname];
                $_SESSION['fname'] = $fname;
                $_SESSION['lname'] = $lname;
                return $data;
            } else {
                $data = ['flag' => false, 'users' => null, 'Courses' => null, 's_id' => null, 'fname' => null, 'lname' => null, 'message' => 'password wrong', 'password' => false, 'email' => true];
                return $data;
            }
        } else {
            $data = ['flag' => false, 'users' => null, 'Courses' => null, 's_id' => null, 'fname' => null, 'lname' => null, 'message' => 'Email not exist', 'password' => false, 'email' => false];
            return $data;
        }
        $data = ['message' => "User Successfully Login", 'user' => "1234"];
        return $data;
    }
}
?>
